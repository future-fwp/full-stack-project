...
const YouTubeVideo = ({ type = "home" }: Props) => {
	...
	const handleVideoSelect = async (video: Video) => {
		...
		await Promise.all([fetchVideoComments(video.id.videoId), fetchRelatedVideos(video.id.videoId)]);
	};
						...
						<div className="space-y-4 max-h-[calc(100vh-100px)] overflow-y-auto">
							{(selectedVideo ? videos.slice(8) : relatedVideos).map((video) => (
								...
								>
									...
									<div className="p-2">
										<h4 className="font-medium line-clamp-2 text-sm">{video.snippet.title}</h4>
